    ...
    <>
      <style>{`
        ...
        .role-btn       { flex: 1; padding: 9px; border: none; background: none; border-radius: 9px;
                          font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; font-family: inherit; color: var(--text-muted); }
        .role-btn.active { background: var(--accent); color: #0a0f1e; box-shadow: 0 0 14px rgba(0,212,255,0.3); }
        ...
        .pass-toggle    { position: absolute; right: 12px; top: 50%; transform: translateY(-50%);
                          background: none; border: none; cursor: pointer; color: var(--text-subtle); padding: 2px; display: flex; }
      ...
      <div className="auth-page">
        <div className="auth-card">
          ...
          <div className="role-toggle">
            <button type="button" className={`role-btn ${role === "BUYER" ? "active" : ""}`} onClick={() => setValue("role", "BUYER")}>
              I want to Buy
            </button>
            <button type="button" className={`role-btn ${role === "VENDOR" ? "active" : ""}`} onClick={() => setValue("role", "VENDOR")}>
              I want to Sell
            </button>
          </div>
